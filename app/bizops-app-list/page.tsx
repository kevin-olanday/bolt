...
import { Search, ChevronUp, ChevronDown, BarChart3, Users, CheckCircle, XCircle, X, Eye } from "lucide-react"
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">BizOps App List</h1>
          <p className="text-gray-600 mt-1">Directory of business applications and infrastructure</p>
        </div>
        ...
        <Card className="bg-slate-50 border-slate-200">
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">Supported by AM</p>
                <p className="text-2xl font-bold text-gray-900">{stats.supportedByAmCount}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-50 border-slate-200">
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">Supported by AC</p>
                <p className="text-2xl font-bold text-gray-900">{stats.supportedByAcCount}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-4">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
              ...
              {/* ARL Tier Filter */}
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">ARL Tier</label>
              ...
              {/* Supported by AM Filter */}
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">AM Support</label>
                <Select value={supportedByAmFilter} onValueChange={setSupportedByAmFilter}>
                  <SelectTrigger className="bg-white border-gray-200">
                    <SelectValue placeholder="AM Support" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">All AM Support</SelectItem>
              ...
              {/* Supported by AC Filter */}
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">AC Support</label>
                <Select value={supportedByAcFilter} onValueChange={setSupportedByAcFilter}>
                  <SelectTrigger className="bg-white border-gray-200">
                    <SelectValue placeholder="AC Support" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">All AC Support</SelectItem>
                    <SelectItem value="true">Supported</SelectItem>
                    <SelectItem value="false">Not Supported</SelectItem>
                  </SelectContent>
                </Select>
              ...
              {/* Business Group Filter */}
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Business Group</label>
              ...
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {visibleColumns.map(({ key, label }) => (
                    <th
                      key={key}
                      className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100"
                  ...
                  <tr key={app.id} className="hover:bg-slate-50">
                    {visibleColumns.map(({ key }) => (
                      <td key={key} className="px-4 py-3 whitespace-nowrap">
                        ...
                        {key === "asset_plan_id" && <span className="text-sm text-gray-900">{app.asset_plan_id}</span>}
                        {key === "cmdb_asset_name" && (
                          <span className="text-sm font-medium text-gray-900">{app.cmdb_asset_name}</span>
                        )}
                        {key === "uam_plan_asset_name" && (
                          <span className="text-sm text-gray-900">{app.uam_plan_asset_name}</span>
                        )}
                        {key === "arl_tier" && (
                        ...
                        {key === "business_group" && (
                          <span className="text-sm text-gray-900">{app.business_group}</span>
                        )}
                        {key === "asset_id" && <span className="text-sm text-gray-900">{app.asset_id}</span>}
                        {key === "sailpoint_name" && (
                          <span className="text-sm text-gray-900">{app.sailpoint_name}</span>
                        )}
                        {key === "request_channel" && (
                          <span className="text-sm text-gray-900">{app.request_channel}</span>
                        )}
                        {key === "revocation_channel" && (
                          <span className="text-sm text-gray-900">{app.revocation_channel}</span>
                        )}
                        {key === "supported_by_am" &&
                          (app.supported_by_am ? (
                        ...
                        {key === "last_modified_date" && (
                          <span className="text-sm text-gray-900">{formatDate(app.last_modified_date)}</span>
                        )}
                        {key === "last_refresh_date" && (
                          <span className="text-sm text-gray-900">{formatDate(app.last_refresh_date)}</span>
                        )}
                      </td>
                    ))}
                  </tr>
